...
def train_mcp_game_subset(env, eval_env, game_name, subset_actions, total_timesteps, log_dir, vid_dir, game_config, num_primitives, features_dim, primitive_action_dim, learning_rate):
    ...
    # In train_mcp_pacman_subset
    pre_training_video_folder = f"{vid_dir}/pre_training_videos"
    os.makedirs(pre_training_video_folder, exist_ok=True)
    pre_training_video_paths = record_gameplay(model, game_name, pre_training_video_folder, subset_actions)
    for i, video_path in enumerate(pre_training_video_paths):
        wandb.log({f"pre_training_gameplay_{i}": wandb.Video(video_path)})
    
    return model, results

def transfer_learning_full_actions(env, eval_env, model_path, game_name, total_timesteps, log_dir, vid_dir, game_config, num_primitives, features_dim, primitive_action_dim, learning_rate):
    wandb.init(project="mcp_atari", name=f"transfer_{game_name}_{log_dir}", config={
        "game": game_name,
        "total_timesteps": total_timesteps,
        "learning_rate": learning_rate
    })
    ...
    # Create a new model with the full action space for the new game
    policy_kwargs = dict(
        features_extractor_class=AtariCNN,
        features_extractor_kwargs=dict(features_dim=features_dim),
        num_primitives=num_primitives,
    ...
    # In transfer_learning_full_actions
    transfer_video_folder = f"{vid_dir}/transfer_videos"
    os.makedirs(transfer_video_folder, exist_ok=True)
    transfer_video_paths = record_gameplay(transferred_model, game_name, transfer_video_folder)
...
def train_baseline_ppo(env, eval_env, total_timesteps, log_dir, game_config):
    wandb.init(project="mcp_atari", name=f"baseline_{log_dir}", config={
        "total_timesteps": total_timesteps
    })
...
def run_experiment(args):
    results = {}
    pre_training_game_config = get_game_config(args.pre_training_game)
    transfer_game_config = get_game_config(args.transfer_game)

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = f"logs/{args.pre_training_game}_to_{args.transfer_game}_primitives{args.num_primitives}_{timestamp}"
    vid_dir = f"/gluster/hdadabhoy/{args.pre_training_game}_to_{args.transfer_game}_primitives{args.num_primitives}_{timestamp}"
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(vid_dir, exist_ok=True)

    # Pre-training environment
    pre_training_env = create_atari_env(args.pre_training_game, args.subset_actions)
    pre_training_eval_env = create_atari_env(args.pre_training_game, args.subset_actions)

    try:
        print(f"Pre-training on {args.pre_training_game}...")
        mcp_subset_model, results["mcp_subset"] = train_mcp_game_subset(
            pre_training_env, pre_training_eval_env, args.pre_training_game, args.subset_actions, 
            args.mcp_subset_timesteps, f"{log_dir}/mcp_subset", f"{vid_dir}/mcp_subset", 
            pre_training_game_config, args.num_primitives, args.features_dim, 
            args.primitive_action_dim, args.pre_training_lr
        )
        print("Pre-training Results:", results["mcp_subset"])
        wandb.finish() 
    ...
    transfer_env = create_atari_env(args.transfer_game)
    transfer_eval_env = create_atari_env(args.transfer_game)

    try:
        print(f"Performing transfer learning to {args.transfer_game}...")
        mcp_full_model, results["mcp_full"] = transfer_learning_full_actions(
            transfer_env, transfer_eval_env, f"{log_dir}/mcp_subset/final_model", 
            args.transfer_game, args.mcp_full_timesteps, f"{log_dir}/mcp_full", 
            f"{vid_dir}/mcp_full", transfer_game_config, args.num_primitives, 
            args.features_dim, args.primitive_action_dim, args.transfer_learning_lr
        )
        print("Transfer Learning Results:", results["mcp_full"])
        wandb.finish() 
    except Exception as e:
    ...
    try:
        print(f"Training baseline PPO on {args.transfer_game}...")
        baseline_model, results["baseline_ppo"] = train_baseline_ppo(
            transfer_env, transfer_eval_env, args.baseline_ppo_timesteps, 
            f"{log_dir}/baseline_ppo", transfer_game_config
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pre_training_game", type=str, default="MsPacman", help="Name of the Atari game for pre-training")
    parser.add_argument("--transfer_game", type=str, default="DonkeyKong", help="Name of the Atari game for transfer learning")
    parser.add_argument("--subset_actions", type=int, nargs="+", default=[1, 2, 3, 4, 5], help="Subset of actions for initial training")
    ...
    parser.add_argument("--transfer_learning_lr", type=float, default=3e-3, help="Learning rate for transfer learning")
    parser.add_argument("--features_dim", type=int, default=512, help="Dimension of the feature extractor output")
    parser.add_argument("--primitive_action_dim", type=int, default=5, help="Dimension of primitive actions")
